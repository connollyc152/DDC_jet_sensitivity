...
def OpenValidationData():
    f = nc.Dataset(data_loc + "Complete_data.nc",'r')
    y = np.array(f['y'][:])
    pf = np.array(f['pf'][:])
    dTdt = np.array(f['Validation_data']['dTdt_validation'][:])
    jet_loc = np.array(f['Validation_data']['jet_lat_validation'][:])
    jet_shift = np.array(f['Validation_data']['jet_shift_validation'][:])
...
for exp in experiment:
    ...
    dataInTraino, dataOutTraino, AO_labelsTo, y, pf = OpenTrainingData()
    ...
    if expsettings['sampleweight'] == "output":
        ...
        plt.hist(weights)
        plt.show()
        plt.scatter(dataOutTraino,  weights, label='training')
        plt.show()
    
    dataInValo, dataOutValo, AO_labelsVo, y, pf = OpenValidationData()
    ...
    if expsettings['SAVESTANDARDIZE']:
        ts = nc.Dataset(data_loc + "standardizingFile_Nshift"+ str(Nshift) +"_"+ str(run_avg) +"runavg" + str(expsettings['TrainingFiles'])+ ".nc", 'w' , format='NETCDF4')
...
#######################################
    ...
    dataInTesto, dataOutTesto, AO_labelsTo, y, pf = OpenTestingData()
