...
import sys
sys.path.insert(0, 'CHANGE TO LCOATION OF FILES')
import Plots as p
...
data_loc = "CHAGNE TO LOCATION OF DATA"
...
def OpenTrainingData():
    f = nc.Dataset(data_loc + "Complete_data.nc",'r')
    y = np.array(f['y'][:])
    pf = np.array(f['pf'][:])
    dTdt = np.array(f['Training_data']['dTdt_training'][:])
    jet_loc = np.array(f['Training_data']['jet_lat_training'][:])
    jet_shift = np.array(f['Training_data']['jet_shift_training'][:])

    print("done Opening data")
    return(dTdt, jet_shift, jet_loc, y, pf) 

def OpenTestingData():
    f = nc.Dataset(data_loc + "Complete_data.nc",'r')
    y = np.array(f['y'][:])
    pf = np.array(f['pf'][:])
    dTdt = np.array(f['Testing_data']['dTdt_testing'][:])
    jet_loc = np.array(f['Testing_data']['jet_lat_testing'][:])
    jet_shift = np.array(f['Testing_data']['jet_shift_testing'][:])
...
#Change path to where model is located
modelsavepath = "CHANGE TO LOCATION OF MODEL"
...
#############################################
model_name = modelsavepath + 'CNN_model' + '.h5'
...
dataInTest, dataOutTest, AO_labelsT, y, pf = OpenTestingData()
...
if STANDARDIZING:
    f = nc.Dataset(data_loc + "standardizingFile_Nshift"+ str(Nshift) +"_"+ str(run_avg) +"runavg" + str(settings['TrainingFiles'])+ ".nc",'r')
    print("Opening " +  data_loc + "standardizingFile_Nshift"+ str(Nshift) +"_"+ str(run_avg) +"runavg" + str(settings['TrainingFiles'])+ ".nc")
...
if AVERAGECHANGE:
    ...
    dataInTrainT, dataOutTrainT, AO_labelsTT, y, pf = OpenTrainingData()    
